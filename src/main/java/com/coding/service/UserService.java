...
public class UserService {
    ...
    public Result<User> addUser(String username, String password, String userRights,String nickname) {
        User record = new User();
        record.setUsername(username);
        List<User> list = userMapper.select(record);
        ...
        record.setPassword(password);
        record.setUserRights(userRights);
    ...
    public Result<User> updateUser(String username, String password, String userRights, String nickname) {
        User record = new User();
        record.setUsername(username);
        List<User> list = userMapper.select(record);
        if (CollectionUtils.isEmpty(list)) {
            return Result.createByErrorMessage("用户不存在无法修改");
        }
        record.setPassword(password);
        record.setUserRights(userRights);
        record.setNickname(nickname);
